define('ds2-need-analyzer',
    [
        'use!jquery',
        'ds2-na-dispatcher',
        'ds2-need-analyzer-input-processor',
        'use!log',
        'ds2-cookie-controller',
        'ds2-main'
    ], function ($, dispatcher, inputProcessor, log) {
        'use strict';
        ...
        NeedAnalyzer.prototype._preloader = function () {
            var preloader = $('.ds2-need-analyzer > .ds2-preloader-wrapper'); //to only delete the one on first level, not all for cosy images
            //enhancement: trigger this rather on animationend
            setTimeout(function () { //TODO why not bind that to image.onLoad
                preloader.remove();
            }, 2000);
        };
        ...
        NeedAnalyzer.prototype._setPaddingBottom = function () {
            ...
            // $(options.needAnalyzer).css('paddingBottom',)
        };
        ...
        NeedAnalyzer.prototype._scrollPosition = function () {
            var self = this,
                ...
                footerOffset = $('.ds2-main-footer').offset(),
                viewport = {};
            viewport.bottom = scrollTop + this.options.windowHeight;

            if (viewport.bottom < footerOffset.top - (options.buttonAreaHeight - options.singleButtonHeight)) {
                this._fixButtonArea();
            }
            else {
        ...
        NeedAnalyzer.prototype._trackStep = function () {
            var self = this;
            var options = self.options;
            var currentStepIndex = options.currentStepIndex;
